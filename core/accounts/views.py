from .serializers import GetMeSerializer,CustomTokenObtainPairSerializer,RegistrationSerializer,ResendTokenSerializer,ChangePasswordSerializer,ResetPasswordSerializer,ResetPasswordConfSerializer
...
class RegistrationApi(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        email_obj = EmailMessage('email/verify.tpl', {'token': token}, 'benxfoxy@gmail.com',to=[email])
...
class Verification(APIView):
    def get(self,request,token,*args,**kwargs):
        try:
            ...
            return Response({'error':'token has been wrong'},status=status.HTTP_400_BAD_REQUEST)
        except DecodeError:
            return Response({'error':'token has been used once'},status=status.HTTP_400_BAD_REQUEST)
...
class ResetPassword(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token = self.get_tokens_for_user(user=user_obj)
        email_obj = EmailMessage('email/resetverify.tpl', {'token': token}, 'benxfoxy@gmail.com',to=[user_obj.email])
        EmailThread(email_obj).start()
...
class ResetPasswordConf(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        self.object.save()
        return Response({'success':'password successfully reset and commited new one'},status=status.HTTP_201_CREATED)